...
function sortPilotsIntoRole(json, randomNumber) {
  ...
  for (i = 0; i < jsonLength; i++) {
    ...
    axisName = firstNamesGermany[imageNumber] + " \"" + json.feed.entry[i].gsx$enterpilotnickname_2.$t + "\" " + lastNamesGermany[imageNumber]; // AXIS pilot chosen position
    ...
    axisBomberSquadronBPosition = json.feed.entry[i].gsx$selectpilotposition_4.$t; // ALLIED Fighter groups

    alliedFG = json.feed.entry[i].gsx$selectflightgroup.$t;
    alliedName = firstNamesUSA[imageNumber] + " \"" + json.feed.entry[i].gsx$enterpilotnickname.$t + "\" " + lastNamesUSA[imageNumber]; // ALLIED pilot chosen position
